...
contract TaiShangAscii is ERC721Enumerable, Ownable{
    ...
    constructor() public ERC721("Tai Shang Ascii", "TSA") {
        // Tai Shang Ascii
    }
    ...
    function mint(string memory url) external returns (uint256){
        uint256 _tokenId = nextTokenId();
        _safeMint(msg.sender, _tokenId);
        tokenURL[_tokenId] = url;
    ...
    function tokenImage(uint256 tokenId) public view returns (string memory) {
        ...
        return string(abi.encodePacked(tokenURL[tokenId]));
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "Nonexistent token");
        string memory name = string(abi.encodePacked('Tai Shang Ascii #', tokenId.toString()));
        string memory description = string(abi.encodePacked(
            'A Tai Shang Ascii token created by ', (uint160(creator[tokenId])).toHexString(20),
            '. To view the artwork, visit ', tokenURL[tokenId]));
